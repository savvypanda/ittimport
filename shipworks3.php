<?php
	...
	{         
		...
		file_put_contents( "shipworks_debug.txt", print_r($_REQUEST, true));
	...
	function WriteOrderItems($orderID)
	{
		...
		$itemQuery = mysql_query(
				...
				" and order_status NOT IN ('I','W','T','R','X')"
		);
		...
		while($item = mysql_fetch_array($itemQuery)) {
			...
			// Get product info //modded for ITTBooks
			$sql = "select p.*". 
				" from ". $dbPrefix. "virtuemart_products p".
				" where p.virtuemart_product_id = " . $item['virtuemart_product_id'];
	...
	function Action_UpdateStatus()
	{
		...
		//make an entry in to order history which will contain the tracking code if available
		mysql_query( 
		...
		//change the overall status of the order
		mysql_query(
				"update ". $dbPrefix. "virtuemart_orders ".
				" set order_status = '$code' " . 
				" where virtuemart_order_id = " . $orderID);

		//update the status and stock for each of the items in the order
		//first get the items in the order
		$orderItems = mysql_query(
				"SELECT virtuemart_product_id, product_quantity
					FROM ". $dbPrefix. "virtuemart_order_items
					WHERE virtuemart_order_id = ".$orderID ."
					AND order_status NOT IN ('I','W','T','R','X')"
		);

		//then change the status of each individual order item
		mysql_query(
				"update ". $dbPrefix. "virtuemart_order_items ".
				" set order_status = '$code' " . 
				" where virtuemart_order_id = " . $orderID .
				" and order_status NOT IN ('I','W','T','R','X')"
		);

		//finally (assuming we are setting the order to shipped), update the inventory for each item
		if($code == 'S') {
			while ($row = mysql_fetch_array($orderItems)) {
				mysql_query(
					"UPDATE ". $dbPrefix. "virtuemart_products ".
					" SET product_in_stock = product_in_stock - 1".
					", product_ordered = product_ordered - 1".
	...
	{
		$sqlDate=new DateTime($sqlDateString);
		return date_format($sqlDate,"Y-m-d\TH:i:s");
	}
