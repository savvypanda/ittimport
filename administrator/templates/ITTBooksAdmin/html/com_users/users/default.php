...
<form action="<?php echo JRoute::_('index.php?option=com_users&view=users');?>" method="post" name="adminForm" id="adminForm">
	...
	<table class="adminlist">
		<thead>
			<tr>
				...
				<th class="nowrap" width="10%">
					<?php echo JHtml::_('grid.sort', 'GUID', 'a.username', $listDirn, $listOrder); ?>
				</th>
                <th class="left">
                	Order History
                </th>
		...
		<?php foreach ($this->items as $i => $item) :
		    ...
		    $display_user = JFactory::getUser($item->id);
			if (in_array(7,$display_user->getAuthorisedViewLevels())) : //7 is for SSCs
				$isssc = true;
			else:
				$isssc = false;
			endif;			
		...
		?>
			<tr class="row<?php echo $i % 2; ?>">
                ...
                <?php 
				if (!$isssc) :?>
                	<a href="index.php?option=com_virtuemart&task=edit&view=user&virtuemart_user_id[]=<?php echo $item->id;?>">Order History</a>
                <?php else:
					if ($isadmin):
					?>
                        <a href="index.php?option=com_vminvoice&controller=invoices&filter_created_by=<?php echo $item->id;?>">
                            Created Orders
                        </a>
                     <?php else: ?>
                     	--
